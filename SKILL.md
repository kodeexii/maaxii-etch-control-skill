...
- **GLOBAL DESIGN SYSTEM (serviswp-)**: Use the prefix `serviswp-` for ALL common and reusable components across the entire website (e.g., `.serviswp-hero`, `.serviswp-card`, `.serviswp-button`). Do not limit this prefix to service pages only. This ensures a unified design system where global changes can be managed from a single source.
